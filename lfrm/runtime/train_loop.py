...
def print_run_overview(
    *,
    ...
    if config.model.model_type in ("trm", "urm"):
        fields.insert(5, ("num_puzzle_identifiers", overview["num_puzzle_identifiers"]))
        fields.insert(18, ("trm_train_mode", config.train.trm_train_mode))
    print(" ".join(f"{name}= {value}" for name, value in fields), flush=True)
